...
BEGIN
  ...
  FOR RECORD IN (SELECT COUNT(DISTINCT B.NAME) AS UNI, SUM(B.AMOUNT) AS REVENUE,TO_CHAR(G.CHECKOUT, 'MONTH') AS CMONTH
   FROM 
      BILL B
      INNER JOIN GUEST G ON B.NAME = G.NAME
    GROUP BY 
      TO_CHAR(G.CHECKOUT, 'MONTH')
  )
  LOOP
    DBMS_OUTPUT.PUT_LINE('MONTH: ' || RTRIM(RECORD.CMONTH) || 
                         ' | COUNT: ' || RECORD.UNI || 
                         ' | REVENUE: ' || RECORD.REVENUE);
  END LOOP;
  SELECT COUNT(DISTINCT B.NAME),SUM(B.AMOUNT) INTO UNI,REVENUE FROM BILL B,GUEST G WHERE B.NAME=G.NAME AND CHECKOUT BETWEEN(SELECT MIN(CHECKIN)  FROM GUEST) AND (SELECT MAX(CHECKOUT)FROM GUEST);
  DBMS_OUTPUT.PUT_LINE('COUNT'||UNI||'REVENUE'||REVENUE);
